...
class TransformerModel(nn.Module):
    def __init__(self, output_dim=NUM_TOKENS, d_model=100, max_len=MAX_INPUT_LEN):
        super(TransformerModel, self).__init__()
        ...
        # Transformer model
        self.transformer = nn.Transformer(d_model, nhead=10, num_encoder_layers=2, num_decoder_layers=8, dim_feedforward=2048, dropout=0.2, batch_first=True)
...
def train_loop(model, opt, loss_fn, dataloader, scaler, scheduler): 
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.amp.autocast("cuda" if torch.cuda.is_available() else "cpu"):
            ...
            pred = model(X, y_input, tgt_mask=tgt_mask, tgt_pad_mask=tgt_pad_mask)
            pred = pred.reshape(-1, OUTPUT_DIMS)
            y_expected = y_expected.reshape(-1)
            y_expected = y_expected.type(torch.LongTensor)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            if len(realX[0]) > 400:
                continue
            for row in X:
                pred.append(inference(model, row, realX[len(pred)]))
...
def inference(model, X, realX):
    ...
    while i < len(realX):
        ...
        # print("predToken", type(predToken), predToken)
        # predToken = pred.topk(1)[1].view(-1)[-1].item()
        if i == len(realX)-1 or (i < len(realX)-2 and realX[i+1] < 3000):
            y.append(predToken)
            y_pred.append(predToken)
        # print("y", y)
        i += 1
...
def collate_fn(batch):
    sequences, labels, realSeq = zip(*batch)  # Unpack inputs and labels
    # print(type(batch))
    # print(type(sequences), type(labels))
    # print(type(sequences[0]))
    sequences = [sequence[:MAX_INPUT_LEN] for sequence in sequences]
    labels = [label[:MAX_INPUT_LEN] for label in labels]
    realSeq = [realSeq[:MAX_INPUT_LEN] for seq in realSeq]
    padded_sequences = pad_sequence(sequences, batch_first=True, padding_value=0)
    padded_labels = pad_sequence(labels, batch_first=True, padding_value=-1)  # Use a different padding value for labels
    return padded_sequences, padded_labels, realSeq
...
class RegisterAllocationDataset(Dataset):
    def __init__(self):
        ...
        with open("vectorized.csv", mode="r") as file:
            reader = csv.reader(file)
            self.data = [torch.tensor([list(map(float, row))], dtype=torch.float) for row in reader]
        ...
        with open("allDataInput.csv", mode="r") as file:
            reader = csv.reader(file)
            self.realData = [torch.tensor(list(map(float, row)), dtype=torch.int32) for row in reader]
    ...
    def __getitem__(self, idx):
        ...
        #     "labels": outputs["input_ids"].squeeze(0),
        # }
        return self.data[idx], self.labels[idx], self.realData[idx]
