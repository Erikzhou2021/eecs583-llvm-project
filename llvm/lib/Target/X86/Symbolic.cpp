...
void MIR2Vec_Symbolic::generateSymbolicEncodings(MachineFunction &F,
  ...
  if (!run) {
    ...
    notBrokenMap = IR2Vec::createNotBrokenMap(F.getTarget().getTargetTriple().getArch());
    run = true;
  }
...
Vector MIR2Vec_Symbolic::bb2Vec(MachineBasicBlock &MB,
  ...
  for (auto &I : MB) {
    ...
    Vector vec(DIM, 0);
    // errs() << notBrokenMap[std::string(TII->getName(I.getOpcode()).data())] << "\n";
    // errs() << TII->getName(I.getOpcode()).data() << "\n";
    // if (getValue(opcDescMap[I.getOpcode()], vec)) {
    if (getValue(notBrokenMap[std::string(TII->getName(I.getOpcode()).data())], vec)) {
      scaleVector(vec, WO);
